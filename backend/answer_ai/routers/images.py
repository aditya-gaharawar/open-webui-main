...
from answer_ai.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from answer_ai.routers.files import upload_file
from answer_ai.utils.auth import get_admin_user, get_verified_user
from answer_ai.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-AnswerAI-User-Name"] = user.name
                headers["X-AnswerAI-User-Id"] = user.id
                headers["X-AnswerAI-User-Email"] = user.email
                headers["X-AnswerAI-User-Role"] = user.role
